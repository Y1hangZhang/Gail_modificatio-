...
class Policy_net:
    def __init__(self, name: str, env):
        ...
        with tf.compat.v1.variable_scope(name):
            ...
            with tf.compat.v1.variable_scope('policy_net'):  #actor
            ...
            with tf.compat.v1.variable_scope('value_net'):  #critic, state의 value 를 리턴한다
                layer_1 = tf.layers.dense(inputs=self.obs, units=20, activation=tf.tanh)
                layer_2 = tf.layers.dense(inputs=layer_1, units=20, activation=tf.tanh)
    ...
    def get_trainable_variables(self):  # Question 트레이너블 변수들 확인하기!
